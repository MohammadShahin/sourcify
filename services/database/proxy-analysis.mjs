...
async function checkProxies() {
  ...
  try {
    ...
    const proxyRpcCount = {
      ...
      "Simple (1)": 0,
      "EIP1967Proxy (1-6)": 0,
      "DiamondProxy (not resolvable by us)": 0, // They can be resolved by each function selector with another implementation
    };
    const proxyTypes = {}
        ...
        for (const row of queryResult.rows) {
          ...
          for (const proxy of whatsabiResult.proxies) {
            if (proxy.name === "FixedProxy") {
              ...
              proxyRpcCount["Simple (1)"]++;
            } else if (
              proxy instanceof whatsabi.proxies.GnosisSafeProxyResolver
            ) {
              proxyRpcCount["Simple (1)"]++;
            } else if (
              proxy instanceof whatsabi.proxies.LegacyUpgradeableProxyResolver
            ) {
              proxyRpcCount["Simple (1)"]++;
            } else if (proxy.name === "EIP1967Proxy") {
              proxyRpcCount["EIP1967Proxy (1-6)"]++;
            } else if (proxy.name === "DiamondProxy") {
              proxyRpcCount["DiamondProxy (not resolvable by us)"]++;
            }
    ...
    console.log("Proxy types by RPC calls for resolution: ", proxyRpcCount);
    ...
    await writeJsonFile("proxy-detection-results.json", resultData);

    await writeJsonFile("proxy-detection-multi-proxy-contracts.json", multiProxyContracts);

    await writeJsonFile("proxy-detection-durations.json", {
    ...
    await writeJsonFile("proxy-detection-durations-stats.json", durationData);
  } catch (error) {
